...
\frame{
  ...
  We are a PHP shop, for what it's worth, and we manage to still be reliable
  thanks to PostgreSQL, used on the front servers:
...
\frame{
  ...
  \begin{itemize}
   \item Web server, EC2, 8GB RAM, 8 CPU, 16 of them
   \item Database servers, EC2, 15GB RAM, 4*400GB local disks
   \item 16 database servers, each hosting 16 databases shards
   \item Cron server, web like, admin server, bdd like
   \item Backup server, EC2, 15GB RAM, 18 EBS (500GB), 18 Hot Standbies
   \item S3 storage for archiving (WAL-E)
  \end{itemize}
}
...
\frame{
  ...
  \begin{itemize}
   \item Streaming data from MySQL to PostgreSQL?
   \item using the MySQL Foreign Data Wrapper
   \item did have to edit the code
   \item very very slow rate
   \item even after optimization tries
  \end{itemize}
}
...
\frame{
  ...
  \begin{itemize}
   \item \texttt{echo \$sql| mysql| psql}
   \item MySQL man page pretends to be sending CSV
   \item But that's a lie.
   \item so we had to write a very simple \texttt{mysql2csv} client
   \item and summon \texttt{pgloader} to the rescue!
  \end{itemize}
}
...
\frame{
  ...
  \begin{itemize}
   \item 37 different mysql sources
   \item loading to temp PostgreSQL where it's all text
   \item \texttt{COPY OUT} from a query with lots of \texttt{COALESCE}
   \item that's where we process \texttt{0000-00-00} dates and the like
   \item oh, and \textit{blobs} too
  \end{itemize}
}
...
\frame{
  ...
  \begin{itemize}
   \item MySQL made it complex to adapt the schema live
   \item and to partition data (13 times the same guestbook table)
   \item finally they opted for \textit{Google Protocol Buffers}
   \item the API is available for Java, C++ and Python
   \item we tried \texttt{pl/python} first, encoding and \texttt{NULL} problems
   \item we then tried \texttt{pl/java}
  \end{itemize}
}
